...
input_images = "./data/full_numpy_bitmap_The Mona Lisa.npy"
...
def discriminator_builder(depth=64, p=0.4): #판별자 생성
    # Define inputs
...
def adversarial_builder(z_dim=100):
    ...
    model.compile(loss='binary_crossentropy', optimizer=RMSprop(lr=0.0004, clipvalue=1.0, decay=3e-8),
                  metrics=['accuracy'])
    return model
...
def train(epochs=2000, batch=128):
    ...
    for i in range(epochs):
        real_imgs = np.resape(data[np.random.choice(data.shape[0], batch, replace=False)], (batch, 28, 28, 1))
        ...
        if (i + 1) % 200 == 0:
            ...
            plt.tight_layout()
            plt.show(
            plt.savefig('./images/MonaLisa_{}.png'.format(i + 1))
    return a_loss, d_loss

a_loss, d_loss = train(epochs=2000)